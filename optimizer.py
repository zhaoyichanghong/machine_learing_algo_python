...
class Momentum:
    ...
    def optimize(self, vars):
        ...
        if not hasattr(self, '_Momentum__v'):
            self.__v = [0] * vars_number

        self.__v = [(1 - self.__alpha) * vars[i] + self.__alpha * self.__v[i] for i in range(vars_number)]
...
class Nesterov:
    ...
    def optimize(self, vars):
        ...
        old_v = self.__v
        self.__v = [self.__alpha * self.__v[i] - self.__learning_rate * vars[i] for i in range(vars_number)]
